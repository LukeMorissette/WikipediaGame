...
class Crawler:
    TIMEOUT = 1000000  # time limit in seconds for the search
    ...
    def find_path(self, start_page, finish_page):
        ...
        while queue and elapsed_time < self.TIMEOUT:  
            (vertex, path, depth) = queue.pop(0)
            for next in set(self.get_links(vertex)) - discovered:
                if next == finish_page:
